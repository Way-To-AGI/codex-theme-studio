...
- The watch injector reapplies after renderer reloads and route changes. An atomic lock enforces one Theme Studio watcher per CDP port, and startup reconciles only exact validated Theme Studio watcher commands before launching a replacement. Its loopback manager owns the mutable active-theme payload so theme changes do not restart the watcher or Codex.
...
- Restore validates exact runtime path, watch mode, and CDP port before terminating any watcher. It waits for watcher exit before removing live CSS/DOM or state, escalates only against the same validated watcher, and leaves Codex, accounts, threads, and app data unchanged.
- Signal, control-API, and renderer-disappearance shutdowns share one deterministic cleanup path. They stop quota polling, close CDP and loopback servers, release the watcher lock, and remove only state owned by the exiting watcher.
