...
try {
  ...
  const listed = await fetch(`http://127.0.0.1:${port}/api/themes`, { headers }); assert.equal(listed.status, 200); assert.equal((await listed.json()).activeTheme, "aurora-focus");
  ...
  console.log(JSON.stringify({ pass: true, checks: 8 }, null, 2));
